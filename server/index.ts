...
export async function initializeRedis(): Promise<Redis | null> {
  ...
  // RAILWAY FIX: Handle Railway's internal Redis URL format
  const isRailway = process.env.RAILWAY_ENVIRONMENT === 'production';
  if (isRailway && redisUrl.includes('redis.railway.internal')) {
    console.log('🔧 RAILWAY: Detected internal Redis URL, attempting to use external URL...');
    
    // Try to get the external Redis URL from Railway
    // Railway provides both internal and external Redis URLs
    // The external URL is typically in the format: redis://default:password@containers-us-west-xxx.railway.app:6379
    const externalRedisUrl = process.env.REDIS_EXTERNAL_URL || process.env.REDIS_PUBLIC_URL;
    
    if (externalRedisUrl) {
      console.log('✅ RAILWAY: Using external Redis URL for better connectivity');
      redisUrl = externalRedisUrl;
    } else {
      console.log('⚠️  RAILWAY: External Redis URL not found, using internal URL with enhanced configuration');
      // We'll proceed with the internal URL but with enhanced configuration
    }
  }
  
  try {
    // RAILWAY FIX: Enhanced Redis configuration for Railway deployment
    const isRailway = process.env.RAILWAY_ENVIRONMENT === 'production';
    const isReplit = process.env.REPL_ID && !process.env.REPLIT_DEPLOYMENT_ID;
    
    console.log('🔧 REDIS: Configuring connection for Railway deployment...');
    console.log(`   Redis URL: ${redisUrl.substring(0, 20)}...`);
    console.log(`   Railway Environment: ${isRailway}`);
    
    // CRITICAL DEBUG: Log full Redis URL for troubleshooting
    ...
    const redisConfig = {
      lazyConnect: true,
      maxRetriesPerRequest: isRailway ? null : 1, // Unlimited retries for Railway
      connectTimeout: isRailway ? 15000 : 3000, // Longer timeout for Railway
      commandTimeout: isRailway ? 10000 : 2000,  // Longer timeout for Railway
      enableAutoPipelining: true,
      enableOfflineQueue: true, // CRITICAL FIX: Enable offline queue for Railway
      retryDelayOnFailover: 100,
      retryDelayOnClusterDown: 300,
      enableReadyCheck: true,
      maxLoadingTimeout: 10000,
      // RAILWAY SPECIFIC: Additional configuration for Railway Redis
      keepAlive: true,
      family: 4, // Force IPv4
      // RAILWAY FIX: Handle Railway's internal Redis URL format
      ...(isRailway && {
        ...
        // Additional Railway-specific socket configuration
        socket: {
    ...
    console.log('🔧 REDIS: Using Railway-optimized configuration');
    console.log(`   Connection timeout: ${redisConfig.connectTimeout}ms`);
    console.log(`   Command timeout: ${redisConfig.commandTimeout}ms`);
    console.log(`   Max retries: ${redisConfig.maxRetriesPerRequest}`);
    ...
    // RAILWAY FIX: Enhanced connection test with Railway-specific timeout and retry logic
    const connectionTimeout = isRailway ? 20000 : 3000;
    console.log(`🔧 REDIS: Testing connection with ${connectionTimeout}ms timeout...`);
    
    // RAILWAY FIX: Multiple connection attempts for Railway
    ...
    while (connectionAttempts < maxAttempts) {
      try {
        connectionAttempts++;
        console.log(`🔧 REDIS: Connection attempt ${connectionAttempts}/${maxAttempts}...`);
        ...
        console.warn(`⚠️  REDIS: Connection attempt ${connectionAttempts} failed: ${attemptError.message}`);
